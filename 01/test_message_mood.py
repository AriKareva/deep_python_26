...
class TestMessageMood(unittest.TestCase):
    def test_if_bad(self):
        self.assertEqual(sl.predict_message_mood("Вулкан"), "неуд")

    def test_if_ok(self):
        t1 = sl.predict_message_mood("Чапаев и пустота", 0.8, 0.99)
        self.assertEqual(t1, "норм")
        self.assertEqual(sl.predict_message_mood("Чапаев11"), "норм")

    def test_if_excellent(self):
        self.assertEqual(sl.predict_message_mood("Чапаев и пустота"), "отл")

    def test_invalid_thresholds(self):
        with self.assertRaises(ValueError):
            sl.predict_message_mood("Чапаев", -0.1, 0.99)

        with self.assertRaises(ValueError):
            sl.predict_message_mood("и", 0.1, -0.99)

        with self.assertRaises(ValueError):
            sl.predict_message_mood("пустота", 0.1, 10.99)

        with self.assertRaises(ValueError):
            sl.predict_message_mood("Вулкан", 10.1, 0.99)
